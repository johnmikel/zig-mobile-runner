...
fn client_drives_fake_session() {
    ...
    assert!(capabilities.methods.iter().any(|method| method == "observe.snapshot"));
    ...
    assert!(client.wait_until(json!({ "text": "Home" }), Some(1000)).unwrap());

    let snapshot = client.snapshot().unwrap();
    assert_eq!(snapshot.active_package, "com.example.mobiletest");
    assert_eq!(snapshot.nodes[0].text.as_deref(), Some("Home"));

    let exported = client.export_trace("traces/rust-client.zmrtrace", true, true).unwrap();
...
fn client_returns_rpc_errors() {
    ...
    let err = client.request::<serde_json::Value>("missing.method", json!({})).unwrap_err();
    match err {
        Error::Rpc(rpc) => {
