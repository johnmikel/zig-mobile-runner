...
test "selector matches resource id and text" {
    ...
    try std.testing.expect(matches(node, .{ .id = "login-button", .text = "Sign in" }));
    try std.testing.expect(!matches(node, .{ .id = "other" }));
}
...
test "selector supports contains matching" {
    ...
    try std.testing.expect(matches(node, .{ .text_contains = "auth" }));
    try std.testing.expect(!matches(node, .{ .text_contains = "missing" }));
}
...
test "selector parser accepts resourceId as an id alias" {
    ...
    const wanted = try parseFromJson(allocator, parsed.value);
