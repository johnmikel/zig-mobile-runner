...
printf 'linux archive bytes\n' > "$DIST/zmr-0.1.0-dev-x86_64-linux-gnu.tar.gz"
...
if (manifest.version !== "0.1.0-dev") throw new Error("unexpected version");
...
for (const required of [
  "zmr-0.1.0-dev-aarch64-macos.15.0.tar.gz",
  "zmr-0.1.0-dev-x86_64-linux-gnu.tar.gz",
  "zig-mobile-runner-0.1.0-dev.tgz",
  "SBOM.spdx.json",
...
if (byPath.get("zmr-0.1.0-dev-aarch64-macos.15.0.tar.gz").type !== "archive") throw new Error("archive type missing");
if (byPath.get("zmr-0.1.0-dev-aarch64-macos.15.0.tar.gz").platform !== "macos") throw new Error("platform missing");
if (byPath.get("zmr-0.1.0-dev-aarch64-macos.15.0.tar.gz").arch !== "aarch64") throw new Error("arch missing");
if (byPath.get("zig-mobile-runner-0.1.0-dev.tgz").type !== "npm-package") throw new Error("npm package type missing");
...
if node "$ROOT/scripts/generate-release-manifest.mjs" --dist "$DIST" --version "0.1.0-dev" --out "$DIST/RELEASE_MANIFEST.json" > "$TMPDIR/missing.out" 2>&1; then
