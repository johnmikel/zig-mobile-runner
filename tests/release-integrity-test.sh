...
printf 'archive bytes\n' > "$DIST_NO_NPM/zmr-0.1.0-dev-aarch64-macos.15.0.tar.gz"
printf '{"spdxVersion":"SPDX-2.3"}\n' > "$DIST_NO_NPM/SBOM.spdx.json"
printf '# Notices\n' > "$DIST_NO_NPM/THIRD_PARTY_NOTICES.md"
printf 'class Zmr < Formula\nend\n' > "$DIST_NO_NPM/homebrew/zmr.rb"
node "$ROOT/scripts/generate-release-manifest.mjs" --dist "$DIST_NO_NPM" --version "0.1.0-dev" --out "$DIST_NO_NPM/RELEASE_MANIFEST.json" >/dev/null
(
  cd "$DIST_NO_NPM"
  shasum -a 256 zmr-0.1.0-dev-aarch64-macos.15.0.tar.gz SBOM.spdx.json THIRD_PARTY_NOTICES.md homebrew/zmr.rb RELEASE_MANIFEST.json > SHA256SUMS
)
...
printf 'archive bytes\n' > "$DIST/zmr-0.1.0-dev-aarch64-macos.15.0.tar.gz"
printf 'npm package bytes\n' > "$DIST/zig-mobile-runner-0.1.0-dev.tgz"
printf '{"spdxVersion":"SPDX-2.3"}\n' > "$DIST/SBOM.spdx.json"
...
(
  cd "$DIST"
  shasum -a 256 zmr-0.1.0-dev-aarch64-macos.15.0.tar.gz zig-mobile-runner-0.1.0-dev.tgz SBOM.spdx.json THIRD_PARTY_NOTICES.md homebrew/zmr.rb RELEASE_MANIFEST.json > SHA256SUMS
)
...
grep -q 'missing checksum entry: zig-mobile-runner-0.1.0-dev.tgz' "$TMPDIR/missing-npm-entry.out"
...
(
  cd "$DIST"
  shasum -a 256 zmr-0.1.0-dev-aarch64-macos.15.0.tar.gz SBOM.spdx.json THIRD_PARTY_NOTICES.md > SHA256SUMS
)
...
(
  cd "$DIST"
  shasum -a 256 zmr-0.1.0-dev-aarch64-macos.15.0.tar.gz SBOM.spdx.json THIRD_PARTY_NOTICES.md homebrew/zmr.rb > SHA256SUMS
)
